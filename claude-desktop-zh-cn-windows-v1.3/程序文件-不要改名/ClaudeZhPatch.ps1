...
try {
  ...
  $sourceAppDir = Get-NormalizedFullPath $install.AppDir
  $targetAppDirNormalized = Get-NormalizedFullPath $targetAppDir
  $targetRootNormalized = Get-NormalizedFullPath $targetRoot
  $mode = 'fixed-copy'

  if ($sourceAppDir.Equals($targetAppDirNormalized, [StringComparison]::OrdinalIgnoreCase)) {
    $mode = 'existing-fixed-copy'
  } else {
    if ($sourceAppDir.StartsWith($targetRootNormalized + '\', [StringComparison]::OrdinalIgnoreCase)) {
      throw 'Only an existing zh-CN copy was found in an unexpected generated path. Reinstall official Claude Desktop, then run this tool again.'
    }
    Write-Info 'Creating a fixed zh-CN app copy. The official Claude install is not modified.'
