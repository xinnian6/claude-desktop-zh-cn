...
安装成功后窗口会自动关闭。如果安装失败，窗口会停住，方便复制错误信息。
...
安装或重新安装简体中文汉化。脚本会复制一份 Claude 到独立的 `Claude-zh-CN` 目录，不直接修改官方原版 Claude。
...
删除本工具生成的汉化版 Claude 和快捷方式，并把语言偏好切回英文。它不会卸载官方原版 Claude。
...
手动调整翻译用。每条都有 `英文原文` 和 `中文译文`，只修改 `中文译文` 即可。
...
正常安装不会添加开机启动项，不会下载网络文件，不会关闭系统虚拟化功能，也不会卸载官方 Claude。

安装时会做这些事：

- 关闭正在运行的 Claude 进程，避免文件被占用。
- 复制官方 Claude 到 `%LOCALAPPDATA%\Programs\Claude-zh-CN`。
- 在复制出来的汉化版里写入中文语言文件。
- 把 Claude 的语言偏好设置为 `zh-CN`。
- 创建 `Claude zh-CN` 快捷方式。

为了减少误传和隐私风险，当前版本不再把官方 `app.asar` 或你的 Claude 配置文件备份到工具目录。
